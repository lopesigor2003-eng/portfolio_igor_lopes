  ...
  return (
    ...
    >
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 sm:pt-12">
        ...
        {/* Dynamic Media Manager/Adder (allows testing images and videos seamlessly) */}
        <div className="mt-16 pt-12 border-t border-white/10 text-left">
          <div className="bg-gradient-to-br from-slate-950 to-indigo-950/20 border border-white/10 rounded-3xl p-6 sm:p-8 space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <div className="space-y-1">
                <h4 className="font-display font-bold text-lg text-white flex items-center gap-2">
                  <Play className="h-4 w-4 text-cyan-400" />
                  Gerenciador Interativo de Conteúdo
                </h4>
                <p className="font-sans text-xs text-slate-400 font-light">
                  Adicione novos criativos (fotos e vídeos) para simular dinamicamente o portfólio em tempo real.
                </p>
              </div>
              <button
                onClick={() => setShowMediaForm(!showMediaForm)}
                className="px-4 py-2 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 text-xs font-semibold text-slate-300 hover:text-white transition-all flex items-center gap-2"
              >
                <span>{showMediaForm ? 'Ocultar Painel' : 'Adicionar Mídia'}</span>
                <Plus className={`h-3.5 w-3.5 transition-transform duration-300 ${showMediaForm ? 'rotate-45' : ''}`} />
              </button>
            </div>
            ...
            {showMediaForm && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4 border-t border-white/5">
                ...
                <form onSubmit={handleAddImage} className="space-y-4">
                  <h5 className="font-display font-bold text-sm text-cyan-300 flex items-center gap-1.5">
                    <ImageIcon className="h-4 w-4" />
                    Adicionar Nova Foto / Capa
                  </h5>
                  <div className="space-y-2">
                    <label className="block text-[10px] font-mono uppercase text-slate-400 font-semibold font-bold">URL da Imagem</label>
                    <input
                      type="url"
                      required
                      ...
                      className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-xs text-slate-200 placeholder:text-slate-600 focus:outline-none focus:border-cyan-400 transition-colors"
                    />
                  </div>
                  <button
                    type="submit"
                    className="w-full py-2 rounded-xl bg-cyan-500 hover:bg-cyan-400 text-slate-950 font-sans font-bold text-xs transition-colors flex items-center justify-center gap-1.5"
                  >
                    <Plus className="h-4 w-4" />
                    <span>Adicionar Peça Gráfica</span>
                  </button>
                </form>

                {/* Add Video Block */}
                <form onSubmit={handleAddVideo} className="space-y-4">
                  <h5 className="font-display font-bold text-sm text-indigo-300 flex items-center gap-1.5">
                    <Video className="h-4 w-4" />
                    Adicionar Novo Vídeo / Campanha
                  </h5>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      <label className="block text-[10px] font-mono uppercase text-slate-400 font-semibold font-bold">Título do Vídeo</label>
                      <input
                        type="text"
                        required
                        placeholder="Ex: Comercial Vestibular"
                        value={newVideoTitle}
                    ...
                    <div className="space-y-2">
                      <label className="block text-[10px] font-mono uppercase text-slate-400 font-semibold font-bold">Link do Vídeo (MP4)</label>
                      <input
                        type="url"
                        required
                  ...
                  <div className="space-y-2">
                    <label className="block text-[10px] font-mono uppercase text-slate-400 font-semibold font-bold font-bold">Breve Descrição</label>
                    <input
                      type="text"
                      placeholder="Ex: Pílula audiovisual para redes sociais"
                      value={newVideoDesc}
                  ...
                  <button
                    type="submit"
                    className="w-full py-2 rounded-xl bg-indigo-500 hover:bg-indigo-400 text-white font-sans font-bold text-xs transition-colors flex items-center justify-center gap-1.5"
                  >
                    <Plus className="h-4 w-4" />
                    <span>Adicionar Campanha Audiovisual</span>
                  </button>
                </form>
              </div>
            )}
